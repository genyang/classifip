...
class OrdinalLogit:
    ...
    def ordinal_logistic_fit(self,X, y, max_iter=10000, verbose=False, solver='TNC'):
        """
    ...
    #     else:
        ...
        out = optimize.minimize(f_obj, x0, args=(X, y), method=solver, jac=f_grad, hessp=f_hess, options=options)
    ...
    def ordinal_logistic_predict(self, w, theta, X):
        """
        Parameters
        ----------
    ...
    def transform_data(self,arff, index):
        labels = arff.attribute_data['class']
        data = [k for i,k in enumerate(arff.data) if i in index]
        X = [row[:-1] for row in data]
        X = np.asarray(X, dtype=np.float)
        y = [labels.index(row[-1]) for row in data]
        y = np.round(y)
        #X -= X.mean()
        y -= y.min()
